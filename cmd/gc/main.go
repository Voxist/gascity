...
func openStoreResultAtForCityWithAuthority(storePath, cityPath string, modeOverride gate.Mode, haveMode, authoritative bool) (beads.StoreOpenResult, error) {
	...
	result, err := beads.OpenStoreAtForCity(context.Background(), beads.StoreOpenOptions{
		...
		OpenNativeStore: func() (beads.Store, error) {
			...
			return openNativeStoreWithIdentityAssertion(context.Background(), scopeRoot, env, nil, beads.WithNativeReopen(reopen))
		},
	})
	if err != nil {
...
// pattern).
// normalizeStoreScopeRoot cleans a scope root and resolves symlinks, matching
// how resolveStoreScopeRoot normalizes. Both sides of any scope-root comparison
// must use it or a linked path (macOS /var -> /private/var) compares unequal to
// the same directory reached directly.
func normalizeStoreScopeRoot(path string) string {
