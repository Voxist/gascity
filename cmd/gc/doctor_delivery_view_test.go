...
func deliveryTestBead(phase string, enteredAgo time.Duration) beads.Bead {
	return beads.Bead{
		Title:  "delivery bead",
		Status: "open",
		Metadata: map[string]string{
			beadmeta.PhaseMetadataKey:          phase,
			beadmeta.PhaseEnteredAtMetadataKey: time.Now().Add(-enteredAgo).UTC().Format(time.RFC3339),
		},
	}
}
...
func TestPRDeliveryCheck_budgetBreach(t *testing.T) {
	...
	mustCreateBead(t, store, deliveryTestBead(deliveryPhaseReviewPending, 65*time.Minute))
...
func TestPRDeliveryCheck_renderExtras(t *testing.T) {
	store := beads.NewMemStore()
	stuckBead := mustCreateBead(t, store, deliveryTestBead(deliveryPhaseReviewPending, 65*time.Minute))
	escalatedB := deliveryTestBead(deliveryPhaseCIPending, 20*time.Minute)
	escalatedB.Metadata[beadmeta.WardenEscalatedMetadataKey] = deliveryPhaseCIPending
...
func TestPRDeliveryCheck_escalationDedupe(t *testing.T) {
	store := beads.NewMemStore()
	b := deliveryTestBead(deliveryPhaseReviewPending, 20*time.Minute)
	b.Metadata[beadmeta.WardenEscalatedMetadataKey] = deliveryPhaseReviewPending
