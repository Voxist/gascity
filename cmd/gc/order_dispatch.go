...
func (m *memoryOrderDispatcher) dispatchWisp(ctx context.Context, store beads.Store, a orders.Order, cityPath, trackingID string) {
	...
	// Best-effort: the BFS fallback covers stores without OrderRunChecker.
	// For checker-backed stores (Doltlite) the SQL-EXISTS fast path relies on
	// these stamps, so a dropped stamp here can under-report open work until
	// the next dispatch re-stamps. Stamps are local Update writes; failures
	// are rare and self-heal on the next materialization pass.
