...
func newMemoryOrderDispatcher(routes *storageRoutes, aa []orders.Order, cityPath string, cfg *config.City, rec events.Recorder, stderr io.Writer) *memoryOrderDispatcher {
	...
	// cfg != nil retained from the fork side: upstream's rename dropped the nil
	// guard, and this is reached on paths that pass a nil cfg.
...
//
// Descendant reasoning is delegated to the orders+graph front door
// (orders.Store.HasOpenWork with the injected wisp-root predicate): a bounded
// number of List calls plus in-memory set operations, never the historical
// O(tree) per-node walk whose subprocess cost blew the per-order gate bound
// under store contention (incident 12 / #2893). Lingering transient nudge/mail
// chores are excluded via isTransientNotificationBead (#2893 #3).
