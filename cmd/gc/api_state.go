...
func (cs *controllerState) CityBeadsDiagnostic() *beads.BeadsDiagnostic {
	...
	// status surfaces reflect the current breaker/cache state. Both fields
	// exist on the wire (BeadsDiagnostic.degraded / gc_bd_inflight) and were
	// left unpopulated by the 2026-08-31 resync, which made them permanently
	// false/zero for every consumer.
...
var controllerDropManagedDoltDatabase = func(cs *controllerState, ctx context.Context, dbName string) error {
	...
	// CityPath, not rig port FILES (city-scale plan P1.7, "port-file
	// de-authority"): on a proxied city the routed store clobbers
