	...
	driveTwice := func(env *reconcilerTestEnv, snap *sessionBeadSnapshot, failing *throttleKeySetMetadataFailStore, carrier *sessionBeadSnapshot) {
		for i := 0; i < 2; i++ {
			reconcileSessionBeadsTracedWithNamedDemand(
				...
				env.dt, nil, nil, nil, map[string]int{"worker": 1}, nil, nil, false, nil, "", nil, env.clk, env.rec, 0, 0,
...
func TestReconcileSessionBeads_Phase0HealVisibleOnSnapshot(t *testing.T) {
	...
	reconcileSessionBeadsTracedWithNamedDemand(
		...
		env.dt, nil, nil, nil, poolDesired, nil, nil, false, nil, "", nil, env.clk, env.rec, 0, 0,
		&env.stdout, &env.stderr, nil,
	)
...
func TestReconcilerUsesLiveRegistry(t *testing.T) {
	...
	woken := reconcileSessionBeadsTracedWithNamedDemand(
		context.Background(), cityPath, snap.OpenForReconcile(), snap, desiredState,
		map[string]bool{"worker": true}, cfg, sp, beads.SessionStore{Store: store}, nil, nil, nil, nil,
		dt, gate, reg, nil, // gate + live registry; no failoverChain
		map[string]int{"worker": 1}, nil, nil, false, nil, "", nil, clk, events.Discard,
