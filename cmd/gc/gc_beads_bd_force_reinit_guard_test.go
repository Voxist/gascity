...
// TestGcBeadsBdInitAdoptsStoreWhoseSchemaBecomesReadable is the other half of
// the guard above, and it exists because the 2026-09-13 resync broke exactly
// this case while every test then present stayed green.
//
// The guard must refuse a store that holds bd tables whose schema NEVER
// becomes readable. It must NOT refuse one whose schema becomes readable a
// moment later. Upstream's version had that property for free: its guard sits
// on bd_runtime_schema_ready's failure path and calls
// wait_for_bd_runtime_schema (8 attempts, 100ms->1s backoff) before refusing.
// Moving the guard into probe_schema_state_or_die's state==1 branch dropped
// the retry, because the fork's probe retries only its UNDETERMINED answer,
// never a confident "table not found: config" -- and that confident answer is
// what a store gives while another writer is mid-bootstrap.
//
// The result was an init that BOTH merge parents survive and the merge killed:
// upstream exits 0 and adopts, the fork exits 0 and reinitializes, the merged
// script died with "stayed unreadable across retries" after performing zero
// retries. Live triggers are a concurrent bd bootstrap, a loaded managed Dolt,
// and the script's own GC_BD_INIT_RETRY=1 re-exec -- which fires PRECISELY
// when wait_for_bd_runtime_schema has just failed and bd init has already
// created the tables.
func TestGcBeadsBdInitAdoptsStoreWhoseSchemaBecomesReadable(t *testing.T) {
	cityPath := t.TempDir()
	if err := os.MkdirAll(filepath.Join(cityPath, ".gc"), 0o755); err != nil {
		t.Fatal(err)
	}
...
`, initMarker)
	...
	// The store holds four bd tables, and its config table is unreadable for
	// the first two probes and readable afterwards -- the concurrent-bootstrap
	// race. A counter file carries the attempt across dolt invocations.
	counter := filepath.Join(t.TempDir(), "config-probe-count")
...
`, counter)
	...
	out := stdout + stderr
	if err != nil {
		t.Fatalf("init refused a store whose bd schema became readable on retry; both merge parents accept this:\n%s", out)
	}
	if strings.Contains(out, "stayed unreadable across retries") {
		t.Fatalf("the refusal fired despite the schema becoming readable:\n%s", out)
	}
	if _, statErr := os.Stat(initMarker); statErr == nil {
		argv, _ := os.ReadFile(initMarker)
		t.Fatalf("init reinitialized a store that was merely slow to answer:\nargv:\n%s\noutput:\n%s", argv, out)
	}
}
