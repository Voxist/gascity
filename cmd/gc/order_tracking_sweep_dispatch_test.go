...
// never answered. If the vc-ny00 sweep filter fails to skip it, any sweep
...
func TestDegradedStoreDoesNotStallOrderDispatch(t *testing.T) {
	...
	// Earlier reads of this scope were killed at their deadline, so its
	// transport breaker is open before the tick runs.
	degradeScope(t, cityPath, rigPath)
