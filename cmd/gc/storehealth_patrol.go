...
// under the scope's .beads directory, for forensic capture during quarantine.
//
// It is a seam. The base build has no db-proxy lifecycle, so the default
// returns none; a build that owns proxied db-proxy children overrides this
// (and reapProxiedChildForScope below) in an init() to wire in live
// process-table discovery and the SIGTERM/grace/SIGKILL reaper.
var proxyChildPIDsForScope = func(string, *config.City, string) []int { return nil }

// reapProxiedChildForScope reaps the given scope's db-proxy child(ren),
// returning the number of PIDs signaled, so a poison in one scope never
// disturbs the others. Seam: default no-op (nothing to reap without a
// db-proxy lifecycle); the proxied build overrides it alongside
// proxyChildPIDsForScope.
var reapProxiedChildForScope = func(string, *config.City, string) int { return 0 }
