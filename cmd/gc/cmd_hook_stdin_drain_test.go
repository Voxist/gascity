...
// executable here is `true` (resolved via LookPath so it works on macOS and
// Linux CI alike), which exits 0 without reading stdin.
...
// executable is `true` (resolved via LookPath so it works on macOS and Linux
// CI alike), which exits 0 without reading stdin.
