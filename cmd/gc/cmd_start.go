...
func doStartStandalone(args []string, controllerMode bool, stdout, stderr io.Writer) int {
	...
	// agent included — may run it, so it does not start the bead store it
	// only describes (ga-fjr5f).
	if !dryRunMode {
	...
	reconcileSessionBeadsAtPathWithNamedDemand(
		...
		nil, awakeAssignedWorkBeads, rigStores, nil, dt, nil, nil, nil, poolDesired,
