...
func (cr *CityRuntime) run(ctx context.Context) {
	...
	if !retryStartupStep("startup", func() bool { return startupComplete }, func() {
		...
		if reapPhantomSessionBeads(cr.sessionsBeadStore().Store, cr.sp, cr.sessionDrains, clock.Real{}, cr.stderr) > 0 {
			sessionBeads = cr.loadSessionBeadSnapshot()
		}
...
func (cr *CityRuntime) tick(
	...
	phaseStart = time.Now()
	phantomReaped := reapPhantomSessionBeads(cr.sessionsBeadStore().Store, cr.sp, cr.sessionDrains, clock.Real{}, cr.stderr)
...
func (cr *CityRuntime) orderTrackingSweepStores() ([]beads.Store, []orderTrackingSweepTarget, func(), error) { //nolint:unparam // targets slice returned for callers that need sweep scope metadata; current call sites discard it
	...
	// vc-ny00: never serialize the tick behind a store whose scope
...
func (cr *CityRuntime) buildDesiredState(sessionBeads *sessionBeadSnapshot, trace *sessionReconcilerTraceCycle) DesiredStateResult {
	...
	// Fork: when a model proxy is configured, rewrite each resolved agent's
	// ANTHROPIC_BASE_URL to route through it. Capture the build result first so
	// this injection applies on top of upstream's class-accessor build.
