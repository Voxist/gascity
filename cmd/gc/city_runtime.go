...
type CityRuntime struct {
	...
	progressEpisodeTracker *progressEpisodeTracker // ADR-0013 A1 M3b; nil until bead reconciler initialized
...
func (cr *CityRuntime) run(ctx context.Context) {
	...
	// Initialize bead-driven reconciler state when bead store is available.
	if cr.cityBeadStore() != nil && cr.tomlPath != "" {
		cr.sessionDrains = newDrainTracker()
		cr.providerHealthGate = newProviderHealthGate()
		cr.progressEpisodeTracker = newProgressEpisodeTracker()
	}
	if ctx.Err() != nil {
		return
	}
...
func (cr *CityRuntime) reloadConfigTraced(
	...
	// Ensure bead-driven reconciler state is initialized when bead store becomes available.
...
func (cr *CityRuntime) beadReconcileTick(ctx context.Context, result DesiredStateResult, sessionBeads *sessionBeadSnapshot, trace *sessionReconcilerTraceCycle) {
	...
	reconcileSessionBeadsTracedWithNamedDemand(
		...
		cr.dops,
		awakeAssignedWorkBeads, rigStores, readyWaitSet, cr.sessionDrains, cr.providerHealthGate, cr.progressEpisodeTracker,
		poolDesired,
