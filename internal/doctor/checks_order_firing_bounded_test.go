...
// Ported from upstream's ga-klv regression guards and adapted to this fork's
// structure: here only the order.fired read goes through the readEvents seam
// (bounded tail + Since window), while the controller-start lookup and the
// never-fired disambiguation probe use events.ReadLatestMatch, which walks
// newest-first with early exit by construction.
...
// eventReadCall records one event-log read issued through the seam.
...
// ga-klv: the check must never issue an unbounded order.fired read against the
// city event log. On a busy city that log reaches hundreds of megabytes, and a
// full scan (36s per read, measured on a 161MB/253k-line log) blows the 15s
// check budget and turns this check permanently red for a reason unrelated to
// order firing.
func TestOrderFiringCurrent_EventReadsAreBounded(t *testing.T) {
	...
	if len(calls) == 0 {
		t.Fatal("check issued no event-log reads through the seam; the spy is not wired")
	}
	for i, call := range calls {
		if call.filter.Type != events.OrderFired {
			t.Fatalf("call %d: unexpected event filter type %q; only order.fired goes through the seam", i, call.filter.Type)
		}
		if call.limit <= 0 {
			t.Fatalf("call %d: order.fired read is unbounded (limit=%d); a full event-log scan blows the check budget", i, call.limit)
		}
		if call.filter.Since.IsZero() {
			t.Fatalf("call %d: order.fired read carries no Since window; the staleness horizon must prune it (vc-89s)", i)
		}
	}
}
...
// already proves current must not be looked up at all. It also guards the
// agreement between the pre-pass and the classification loop — a filter drift
// that dropped the stale order from the prefetch set would surface here as a
// second, inline lookup.
