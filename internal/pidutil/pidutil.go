...
// exposed (CONFIG_PROC_CHILDREN off), so the caller falls back to the next
// mechanism — sysctlChildPIDs on darwin, ps everywhere else. A parent
...
func ChildPIDs(parent int) ([]int, error) {
	...
	if children, ok := sysctlChildPIDs(parent); ok {
		return children, nil
	}
...
//
// The deadline is psTimeout() (#5427), not a fixed enumeration budget: this is
// the one ps call that genuinely reads the whole process table, so it is the
// most exposed to the cost that scales with it.
