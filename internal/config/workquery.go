...
func routedReadyTierCommand(topo QueryTopology) string {
	...
	// age-based anti-starvation property that `--sort oldest` was chosen for
	// AND stops a fresh P1 from starving behind an older P2/P3 — under
	// `oldest`, priority was decorative for the pool tier. hybrid's 48h seam
	// is a bd 1.1.0 constant (internal/storage/sqlbuild/ready.go).
	//
	// The count-form predicate (poolDemandCountShell) deliberately passes no
	// --sort — order is irrelevant to a length — so this ordering choice does
