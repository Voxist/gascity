...
func TestRoutedReadyTierCommandServesTopPriorityBehindDeepBacklog(t *testing.T) {
	...
	// non-P0 priority for a minimal fixture). The window read (--sort hybrid
	// since the ADR-0035 rebase; the aged set here is past the 48h seam, where
	// hybrid IS oldest-first) returns this set; the priority read (--sort
	// priority) returns the P0 that the window cannot reach.
	...
	fakeBd := strings.Join([]string{
		...
		`  *"--sort hybrid"*) printf '%s' ` + shellQuoteForScript(string(agedJSON)) + `;;`,
