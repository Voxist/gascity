...
func LoadWithIncludesOptions(fs fsys.FS, path string, opts LoadOptions, extraIncludes ...string) (*City, *Provenance, error) {
	...
	if len(root.Patches.Agents) > 0 {
		...
		for i, p := range root.Patches.Agents {
			...
			deferPatch := false
			resolvable := p.Name == "" || agentPatchMatchesExisting(root, &p)
			if !resolvable {
			...
			case resolvable:
				nowPatches = append(nowPatches, p)
			default:
				// vc-9wa / vc-quqf: a patch naming an agent absent from the
				// merged config is SKIPPED WITH A WARNING, not a hard error.
				// One bad patch target aborting config load took a whole city
				// down. gc config lint still treats the warning as a failure
				// (IsUnresolvedAgentPatchWarning), so a typo is caught at
				// pre-commit rather than at city start.
				prov.Warnings = appendUnique(prov.Warnings,
	...
	if err := ValidateCityRole(root, path); err != nil {
		return nil, nil, err
	}
