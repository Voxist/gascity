...
func TestMergeOracleFieldCoverage(t *testing.T) {
	...
	excludedStore := map[string]bool{
		...
		"degradedReads": true,
		// The vc-ny00 store-warming tracker, excluded on the same argument as
		// the availability-gate fork directly above: it is about backing-store
		// LATENCY, not durable cache content. warm points at the
		// process-global tracker keyed by this store's id prefix
		// (store_warming.go) — a shared collaborator, not cache state, and
		// comparing a pointer to shared mutable state would compare the
		// registry rather than the merge. It is not written by
		// mergeSnapshotLocked and the merge end state does not depend on it;
		// its behavior is pinned by the store-warming suite.
		"warm": true,
