...
func TestMergeOracleFieldCoverage(t *testing.T) {
	...
	excludedStore := map[string]bool{
		...
		// Read-path/transport-gate state, orthogonal to the reconcile bead-state
		...
		"availabilityGate": true, "unavailableSkipLogged": true, "degradedReads": true,
	}
	...
	excludedStats := map[string]bool{
		...
		// Read-path counter (last-good serves while the gate reports the
		// backing unavailable), not a reconcile delta — mirrors excluded
