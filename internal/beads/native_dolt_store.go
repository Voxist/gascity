...
type NativeDoltStore struct {
	...
	actor      string
	idPrefix   string
	// projectID is the project_id the store reported at open, read from the
	// opened storage's config table. An opened-but-typeless embedded DB (the
	// silent-misroute signature) leaves this empty, which the post-open identity
	// assertion surfaces as opened-empty.
	projectID string
...
var (
	...
	_ StorageCreateStore            = (*NativeDoltStore)(nil)
