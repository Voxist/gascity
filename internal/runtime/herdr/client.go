...
// herdrConfigDir resolves herdr's config root using STANDARD XDG PRECEDENCE on
// every platform, rather than delegating to os.UserConfigDir.
//
// os.UserConfigDir is XDG-correct on Linux but NOT on darwin, where it returns
// $HOME/Library/Application Support and ignores XDG_CONFIG_HOME entirely
// (verified 2026-08-31: with XDG_CONFIG_HOME=/tmp/xdgprobe it still answers
// "$HOME/Library/Application Support"). herdr itself uses XDG precedence on all
// platforms — see the socketPath doc below for the empirical check — so on
// darwin the client dialed a socket herdr never binds. That is ga-nqlb8q's
// failure mode, unfixed on macOS: serverAlive() reads false against a healthy
// server, and every retry launches a redundant herdr contending for the same
// pane ("agent_pane_busy").
func herdrConfigDir() string {
	...
	configDir, _ := os.UserConfigDir() // last resort; platform-specific
	return configDir
}
