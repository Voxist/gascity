...
const (
	...
	//
	// The fork briefly re-pointed this to vp-8eqrh (vp-d246m) on the grounds
	// that ga-80po0c.3 "does not resolve via `bd show` in this repo's own
	// store". Checked 2026-08-31: NEITHER id resolves here (`bd show` errors on
	// both), so the stated reason did not hold and the re-point bought nothing.
	// Reverted to upstream's pin rather than carrying a fork-only owner that is
	// equally un-naggable.
