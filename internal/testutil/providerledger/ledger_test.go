...
func TestCatalogReturnsIndependentEntries(t *testing.T) {
	...
	// Find a waived claim rather than assuming an index: the ledger's entry
	// order and dispositions change as contracts get proved (acp.NewSeamBacked
	// moved waived -> proved), and a hardcoded index nil-derefs the moment it
	// lands on a proof.
	waivedIdx, waivedClaim := -1, -1
	for i := range first {
		for j := range first[i].Claims {
			if first[i].Claims[j].Waiver != nil {
				waivedIdx, waivedClaim = i, j
				break
			}
		}
		if waivedIdx >= 0 {
