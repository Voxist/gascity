...
## Iteration-loop aid: a 7-file change should not run ~190 packages. Shares its
## selection with lint-affected (scripts/ci-static-select) and falls back to
## ./... on any uncertainty. The pre-push hook and CI still run everything, so
## a selection miss costs time, never correctness.
test-affected:
