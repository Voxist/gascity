...
## test-affected: run go test over only the packages this change can affect
## Iteration-loop aid, NOT a gate. Shares its selection with lint-affected
## (scripts/ci-static-select) rather than growing a second, weaker
## change-detection, and falls back to ./... on any uncertainty. The pre-push
## gauntlet and CI still run the full tiers, so a selection miss costs time,
## never correctness. Value varies: narrow for a leaf package, broad for a
## widely-depended-on one, because reverse dependents are included.
test-affected:
	@GOFLAGS="$(QUALITY_GATE_GOFLAGS)" "$(CI_STATIC_SELECT)" test-affected "$(CI_STATIC_GO)" $(GO_TEST_FLAGS)
...
test-ci-policy:
	...
	$(TEST_ENV) bash scripts/test-tier-coverage.sh
