...
func TestAgentImageRebuildsBDAndGCWithPatchedGRPC(t *testing.T) {
	const (
		...
		// fixed 0.40.0). The pinned source now declares x/crypto 0.56.0 and
		// x/mod 0.40.0, so these sit at parity with it rather than above it.
...
func TestMCPMailImagePinsPatchedPythonDependencies(t *testing.T) {
	...
	// its own header; these are the resolved versions that command produces, and
	// each must stay at or above the floor above. Bumping a floor without
