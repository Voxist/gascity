...
func TestBDVersionPins(t *testing.T) {
	...
	if bridgeRef == "" {
		...
		// source-built agent image must all use the same upstream beads. A drift
		...
		// BD_CURRENT_VERSION verbatim (BD_CURRENT_REF is then that tag's commit,
		// which only the network could confirm -- the same trust boundary the
		// pseudo-version form already had for its timestamp).
...
// on upstream's tag namespace, so the export is required regardless.
//
