...
func TestDoltConfigWiringExternalHost(t *testing.T) {
	...
	runBDInitCompat(t, env, wsDir, doltConfigIssuePrefix, port, doltConfigSharedDatabase)
	...
	// second machine's agent sharing the same bead store. --database is the
	// documented way to join a database another tool already created, and the
	// same invocation gascity's own rig init uses (initDefaultRigBdStore in
	// cmd/gc/beads_provider_lifecycle.go). Without it bd mints a fresh project
	// ID for this workspace and then refuses to open the existing database
	// (PROJECT IDENTITY MISMATCH) — its guard against silently adopting a
	// foreign project's data, not a cross-workspace sharing failure.
	runBDInitCompat(t, env, wsDir2, doltConfigIssuePrefix, port, doltConfigSharedDatabase)
