...
sync_database_sql() {
  ...
  if [ -z "$remote_pair" ] || [ "$remote_pair" = "AMBIGUOUS" ]; then
    # gc-6010 made "no remote to sync against" two distinct states: no remote
    # is configured at all, and remotes exist but none is local and no
    # GC_DOLT_REMOTE_<DB> override picked one (AMBIGUOUS). Both mean nothing
    # was pushed, so both take the ADR-0064 branch below; only the wording
    # differs.
    if [ "$remote_pair" = "AMBIGUOUS" ]; then
      no_remote_detail="no local remote; set GC_DOLT_REMOTE_$(db_env_key "$name" 2>/dev/null) to push to a non-local remote"
    else
      no_remote_detail="no remote"
    fi
    # ADR-0064 D3/AC4. Outside the delivery window this is a benign "nothing to
    # do". Inside it, a store with no remote to push to is the P0 condition
    ...
    if [ "$drain" = true ]; then
      echo "  $name: NO REMOTE — no off-box copy is possible for this store ($no_remote_detail); configure a remote or mark it .no-sync" >&2
      undrained_stores="$undrained_stores $name"
      return 1
    fi
    echo "  $name: skipped ($no_remote_detail)"
    return 0
  fi
